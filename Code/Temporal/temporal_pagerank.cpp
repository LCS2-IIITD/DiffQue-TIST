...
struct node
{
   ll asker, answerer;
   bool best;
   ll time_asker, time_answerer;
} graph[5000010];
map<ll, ll> outgoinglinks;
double score1[5000010], score2[5000010], score3[5000010];
map< ll, map<ll, ll> > timewise;
int main()
{
    freopen("edge_with_timestamp.txt", "r", stdin);
    freopen("pagerank_with_weight.txt", "w", stdout);

    ll U, V, T1, T2, bestt, N, i, j, No_Of_Buckets = 100;

    cin >> N;
    rep(i, 1, N)
    {
       cin >> U >> V >> bestt >> T1 >> T2;     // T1, T2 in form of buckets
       ...
       graph[i].answerer = V;
       graph[i].best = bestt;
       graph[i].time_asker = T1;
       graph[i].time_answerer = T2;
    }
    ...
    rep(i, 1, N)
    {
        ...
        V = graph[i].answerer;
        bestt = graph[i].best;
        T1 = graph[i].time_asker;
        T2 = graph[i].time_answerer;
    ...
    rep(i, 1, N)
    {
        ...
        double avg = ((double)outgoinglinks[U])/No_Of_Buckets, var = 0;
        tr(timewise[U], it)
        {
            double val = ( avg - timewise[U][it->f] );
            val *= val;
    ...
    rep(i, 1, N)
    {
        U = graph[i].asker;
        V = graph[i].answerer;
        double score = (alpha*score1[i]) + (beta*score2[i]) + (gamma*score3[i]);

        cout << U << " " << V << " " << fixed << setprecision(12) << score << endl;
    }
    // Final Pagerank on new weights
