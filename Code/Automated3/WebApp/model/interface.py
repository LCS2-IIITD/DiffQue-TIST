...
def getResults(id1, id2):
	#url1 = "https://api.stackexchange.com/2.2/questions/" + str(id1) + "?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T'https://api.github.com/users?since=100"
	#url2 = "https://api.stackexchange.com/2.2/questions/" + str(id2) + "?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T'https://api.github.com/users?since=100"

        #r1 = requests.get(url1)
        #r2 = requests.get(url1)

        #id1 = str(29134512)
	data1 = requests.get('https://api.stackexchange.com/2.2/questions/' + id1 + '?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T').json()
	token1 = data1['items'][0]['body_markdown']

        #id2 = str(29134512)
        data2 = requests.get('https://api.stackexchange.com/2.2/questions/' + id2 + '?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T').json()
        token2 = data2['items'][0]['body_markdown']

        #data1 = requests.get('https://api.stackexchange.com/2.2/questions/' + str(id1) + '?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T').json()
        #data2 = requests.get('https://api.stackexchange.com/2.2/questions/' + str(id2) + '?order=desc&sort=activity&site=stackoverflow&filter=!9YdnSJ*_T').json()

        token1 = data1['items'][0]['body_markdown']
        token2 = data2['items'][0]['body_markdown']

        token1 = token1.replace('\r', "")
