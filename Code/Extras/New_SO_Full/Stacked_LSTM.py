...
print('Defining')
model = Sequential()
model.add(LSTM(nooffeatures, input_shape=(1, nooffeatures), return_sequences=True))
#model.add(Dropout(0.5))
model.add(LSTM(nooffeatures, input_shape=(1, nooffeatures), return_sequences=True))
model.add(Flatten())
model.add(Dense(nooffeatures, activation='relu'))
#model.add(Dropout(0.5))
#model.add(Dense(nooffeatures, activation='softmax'))
model.add(Dense(1, activation='sigmoid'))
model.compile(loss='mean_absolute_error', optimizer='adam', metrics=['accuracy'])
print('Training')
model.fit(trainvector, trainlabel, epochs = 2, batch_size=64)
